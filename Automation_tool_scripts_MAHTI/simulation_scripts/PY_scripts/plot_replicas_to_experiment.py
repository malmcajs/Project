...
def calculate_coil_probability(path, output, stride=20):
    ...
    for file in path:
        traj_file = file + "noPBC.xtc"
        
        gro_file = file + "first_frame.gro"
...
def relaxation_combined(sim, exp, output, execution):
	...
	for idx, item in enumerate(Names):
		...
		if rep_name + "/" + forcefield in Best_cases:
			...
			print(autoAxis)
			rec = Rectangle((autoAxis[0]-(autoAxis[1] - autoAxis[0]) * 0.00, autoAxis[2]- (autoAxis[3] - autoAxis[2]) * 0.00), 
					(autoAxis[1] - autoAxis[0])*1.0, 
					(autoAxis[3] - autoAxis[2])*1.0, 
	...
	for path in data_path:
		...
		if same==False:
			...
			if name in Best_cases:
				...
				print(autoAxis)
				rec = Rectangle((autoAxis[0]-(autoAxis[1] - autoAxis[0]) * 0.00, autoAxis[2]- (autoAxis[3] - autoAxis[2]) * 0.00), 
					(autoAxis[1] - autoAxis[0])*1.00, 
					(autoAxis[3] - autoAxis[2])*1.00, 
					fill=False, linewidth=5, edgecolor='black')
				axs[i, j].add_patch(rec)
				rec.set_clip_on(False)
    ...
    for i, rep in enumerate(REPLICAS):
            for j, ff in enumerate(FORCEFIELDS):
                ...
                if os.path.exists(file):
                    ...
                    if rep + "/" + ff in Best_cases:
                        autoAxis = axs[i, j].axis()
                        rec = Rectangle((autoAxis[0]-(autoAxis[1] - autoAxis[0]) * 0.03, autoAxis[2]*1.3**(-1)), 
                            (autoAxis[1] - autoAxis[0])*1.06, 
                            (autoAxis[1] - autoAxis[0])*1.3, 
                            fill=False, linewidth=5, edgecolor='black')
                        axs[i, j].add_patch(rec)
                        rec.set_clip_on(False)                        
...
#	plt.tight_layout()
	plt.savefig(output + "Avg_contact.pdf", dpi=600)
	plt.close()
...
def corr_map_avg(input, output):
	...
	plt.yticks([])
#	plt.colorbar()
	plt.savefig(output + "Avg_corr.pdf", dpi=600)
	plt.close()
...
#	plt.tight_layout()
	...
	plt.savefig(output_file + "Avg_dist.pdf", dpi=600)
	plt.close()
...
def run_pymol_operations():
	...
	for i in pdb_data:
		if i.split('/')[-3]+ "/" + i.split('/')[-2] in Best_cases:
			...
			md = glob.glob(i + 'noPBC.xtc')[0]
			temp = glob.glob(i + 'first_frame.gro')[0]
	...
	for case in pdb_data:	
		...
		print(selected)
		md = glob.glob(case + 'noPBC.xtc')[0]
		temp = glob.glob(case + 'first_frame.gro')[0]
		...
		second_path = case + 'Secondary probability ' + rep_name + '_' +  forcefield + '.png'
